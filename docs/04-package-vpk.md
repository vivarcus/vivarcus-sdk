...
**编号示例（01–09）不要走本文。** 它们用 `vivarcus sdk put -f <子目录>/<类型名>.go`，见 [05-deploy](05-deploy.md)。本文只给 [multi-component](../examples/multi-component) 这类多文件树。

客户多入口 Record Action 通过 **Inbound VPK** 部署。结构：**可选 `components/` MDL 步** + 末尾 **`gosdk/`**（仅 `.go`）。

本地 `go.mod` 只给 `go get` / `go test` 用，**不要**放进 VPK。多包布局不要放根 `main.go`。客户 module path 写在 `vaultpackage.xml` 的 `<gosdk><module>`；平台编译时按当前镜像生成 `go.mod`，并由 reactor 生成唯一的 `func main`。`package-vpk.sh` 会跳过 `*_test.go`、`zz_generated_reactor.go`，以及只含空导入的根 `main.go`。
...
- 校验 `gosdk/` 只含 `.go`（**拒绝** `.wasm`、`go.mod`、`go.sum`、薄 `main.go`，以及任何客户 `func main`）
...
手打时 **不要** `cp go.mod dist/gosdk/`。客户 `.go` 里不要写 `func main`，多包布局不要放只做空导入的根 `main.go`。
