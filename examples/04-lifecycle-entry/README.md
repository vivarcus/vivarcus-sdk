...
**不要打 VPK。** 先 `sdk put` 投影 `Recordaction`，再 apply 引用该 FQN 的生命周期 / workflow MDL。多文件 combo 见 [multi-component](../multi-component)。
...
不要上传 `zz_generated_reactor.go`（`sdk put` 只传 `main.go`）。
