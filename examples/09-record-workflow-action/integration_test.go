...
func TestRecordWorkflowActionIntegration(t *testing.T) {
	env := itest.Open(t)
	env.ApplyObjectMDLIfMissing(t, "capture_demo__c", "mdl/01-object.mdl")
	...
	rec := env.CreateRecord(t, "capture_demo__c", map[string]any{"name__v": "capture-participants"})
	started := env.Transition(t, "capture_demo__c", rec, "start_capture_wf__c")
	if started == nil {
